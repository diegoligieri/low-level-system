...
DeathLostExperienceStages loadDeathLostExperienceStages(lua_State* L)
{
	DeathLostExperienceStages stages;

	lua_getglobal(L, "deathLostExperienceStages");
	if (!lua_istable(L, -1)) {
		return {};
	}
	...
	while (lua_next(L, -2) != 0) {
		...
		auto minLevel = LuaScriptInterface::getField<uint32_t>(L, tableIndex, "minlevel");
		auto maxLevel = LuaScriptInterface::getField<uint32_t>(L, tableIndex, "maxlevel");
		auto value = LuaScriptInterface::getField<float>(L, tableIndex, "value");
		stages.emplace_back(minLevel, maxLevel, value);
		lua_pop(L, 4);
	}
	lua_pop(L, 1);
...
DeathLostMagicLevelStages loadDeathLostMagicLevelStages(lua_State* L)
{
	DeathLostMagicLevelStages stages;

	lua_getglobal(L, "deathLostMagicLevelStages");
	if (!lua_istable(L, -1)) {
		return {};
	}
...
DeathLostSkillsStages loadDeathLostSkillsStages(lua_State* L)
{
	DeathLostSkillsStages stages;

	lua_getglobal(L, "deathLostSkillsStages");
	if (!lua_istable(L, -1)) {
		return {};
	}
...
bool ConfigManager::load()
{
	...
	boolean[DEFAULT_POTIONS_RUNES_SPELLS_BEHAVIOR] = getGlobalBoolean(L, "defaultPotionsRunesSpellsBehavior", false);
	boolean[USE_POTIONS_AND_RUNES_AT_SAME_TIME] = getGlobalBoolean(L, "usePotionsAndRunesAtSameTime", false);
	boolean[POTIONS_BLOCK_INSTANT_SPELLS] = getGlobalBoolean(L, "potionsBlockInstantSpells", false);
	boolean[OTHERS_BLOCK_RUNES] = getGlobalBoolean(L, "othersBlockRunes", false);
	boolean[OTHERS_BLOCK_POTIONS] = getGlobalBoolean(L, "othersBlockPotions", false);
	boolean[OTHERS_BLOCK_SPELLS] = getGlobalBoolean(L, "othersBlockSpells", false);
	boolean[OTHERS_BLOCK_MOVECREATURE] = getGlobalBoolean(L, "othersBlockMoveCreature", false);
	boolean[EQUIP_FLOOR_ITEM] = getGlobalBoolean(L, "equipFloorItem", false);
	...
	integer[KILLS_TO_BANIMENT] = getGlobalNumber(L, "killsToBaniment", 6);
	integer[BANIMENT_TIME] = getGlobalNumber(L, "banimentTime", 24 * 60 * 60);
	...
	integer[SKULL_LEVEL_DIFF] = getGlobalNumber(L, "skullLevelDiff", 200);
	...
	integer[MONSTERS_ATTACK_INTERVAL] = getGlobalNumber(L, "monstersAttackInterval", 2000);
	integer[MONSTERS_HEAL_INTERVAL] = getGlobalNumber(L, "monstersHealInterval", 2000);
	integer[MONSTERS_ATTACK_MELEE_BOOST_PERCENT] = getGlobalNumber(L, "monstersAttackMeleeBoostPercent", 0);
	integer[MONSTERS_DEFENSE_MELEE_BOOST_PERCENT] = getGlobalNumber(L, "monstersDefenseMeleeBoostPercent", 0);
	integer[MONSTERS_ATTACK_PHYSICAL_BOOST_PERCENT] = getGlobalNumber(L, "monstersAttackPhysicalBoostPercent", 0);
	integer[MONSTERS_DEFENSE_PHYSICAL_BOOST_PERCENT] = getGlobalNumber(L, "monstersDefensePhysicalBoostPercent", 0);
	integer[MONSTERS_ATTACK_SPELL_BOOST_PERCENT] = getGlobalNumber(L, "monstersAttackSpellBoostPercent", 0);
	integer[MONSTERS_DEFENSE_SPELL_BOOST_PERCENT] = getGlobalNumber(L, "monstersDefenseSpellBoostPercent", 0);
	integer[MAXIMUM_SPEC_CAST] = getGlobalNumber(L, "maximumSpectatorsCasting", 50);
	integer[MIN_PUSH_LEVEL] = getGlobalNumber(L, "minPushLevel", 100);
	integer[PUSH_CREATURE_DELAY_FAR] = getGlobalNumber(L, "pushCreatureDelayFar", 1000);
	integer[PUSH_CREATURE_DELAY_CLOSE] = getGlobalNumber(L, "pushCreatureDelayClose", 1000);
	integer[LOSS_PER_BLESS] = getGlobalNumber(L, "lossPerBless", 10);
	...
	deathLostExperienceStages = loadDeathLostExperienceStages(L);
	deathLostMagicLevelStages = loadDeathLostMagicLevelStages(L);
	deathLostSkillsStages = loadDeathLostSkillsStages(L);
...
float ConfigManager::getDeathLostExperienceStage(uint32_t level) const
{
	auto it = std::find_if(deathLostExperienceStages.begin(), deathLostExperienceStages.end(), [level](DeathLostExperienceStages::value_type stage) {
		return level >= std::get<0>(stage) && level <= std::get<1>(stage);
	});

	if (it != deathLostExperienceStages.end()) {
		return std::get<2>(*it);
	}

	if (!deathLostExperienceStages.size()) {
		return 0;
	}

	return std::get<2>(deathLostExperienceStages[deathLostExperienceStages.size() - 1]);
}

float ConfigManager::getDeathLostMagicLevelStage(uint32_t level) const
{
	auto it = std::find_if(deathLostMagicLevelStages.begin(), deathLostMagicLevelStages.end(), [level](DeathLostMagicLevelStages::value_type stage) {
		return level >= std::get<0>(stage) && level <= std::get<1>(stage);
	});

	if (it != deathLostMagicLevelStages.end()) {
		return std::get<2>(*it);
	}

	if (!deathLostMagicLevelStages.size()) {
		return 0;
	}

	return std::get<2>(deathLostMagicLevelStages[deathLostMagicLevelStages.size() - 1]);
}

float ConfigManager::getDeathLostSkillsStage(uint32_t level) const
{
	auto it = std::find_if(deathLostSkillsStages.begin(), deathLostSkillsStages.end(), [level](DeathLostSkillsStages::value_type stage) {
		return level >= std::get<0>(stage) && level <= std::get<1>(stage);
	});

	if (it != deathLostSkillsStages.end()) {
		return std::get<2>(*it);
	}

	if (!deathLostSkillsStages.size()) {
		return 0;
	}

	return std::get<2>(deathLostSkillsStages[deathLostSkillsStages.size() - 1]);
}