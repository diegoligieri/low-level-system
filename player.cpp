...
Player::Player(ProtocolGame_ptr p) : Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), client(new Viewer(p))
{
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	Tile* thisTile = thisPlayer->getTile();
	if (!thisTile || (!thisTile->hasFlag(TILESTATE_PROTECTIONZONE) && playerTile->hasFlag(TILESTATE_PROTECTIONZONE))) {
		return false;
	}
...
void Player::onThink(uint32_t interval)
{
	...
	if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
		...
		if (idleTime > (kickAfterMinutes * 60000) + 60000) {
			kickPlayer(true);
		} else if (hasClient() && idleTime == 60000 * kickAfterMinutes) {
...
void Player::death(Creature* lastHitCreature)
{
	...
	if (skillLoss) {
		...
		uint64_t manaSpent = static_cast<uint64_t>(vocation->getReqMana(magLevel) * (g_config.getDeathLostMagicLevelStage(magLevel) / 100.f));
		removeManaSpent(manaSpent - ((manaSpent * (lossPerBless * g_config.getNumber(ConfigManager::LOSS_PER_BLESS))) / 100.f), false);
		...
		for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) { //for each skill
			...
			uint64_t skillTries = vocation->getReqSkillTries(i, skills[i].level) * (g_config.getDeathLostSkillsStage(skills[i].level) / 100.f);
			removeSkillTries(static_cast<skills_t>(i), skillTries - ((skillTries * (lossPerBless * g_config.getNumber(ConfigManager::LOSS_PER_BLESS))) / 100.f), false);
		}

		//Level loss
		uint64_t expLoss = Player::getExpForLevel(level) - Player::getExpForLevel(level - g_config.getDeathLostExperienceStage(level));
		expLoss = expLoss - ((expLoss * (lossPerBless * g_config.getNumber(ConfigManager::LOSS_PER_BLESS))) / 100.f);
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if (inventoryItem && (!inventoryItem->isStackable() || inventoryItem->getID() != item->getID())) {
		...
		return !g_config.getBoolean(ConfigManager::EQUIP_FLOOR_ITEM) ? RETURNVALUE_NOTENOUGHROOM : RETURNVALUE_NEEDEXCHANGE;
	}
	return ret;
}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	BanInfo banInfo;
	if (getSkull() == SKULL_RED && g_config.getNumber(ConfigManager::KILLS_TO_BANIMENT) != 0 && skullTicks > (g_config.getNumber(ConfigManager::KILLS_TO_BANIMENT) - 1) * static_cast<int64_t>(g_config.getNumber(ConfigManager::FRAG_TIME))) {
		IOBan::insertAccountBan(accountNumber, "for excessive frags", time(nullptr), time(nullptr) + g_config.getNumber(ConfigManager::BANIMENT_TIME), guid);
		kickPlayer(true);
